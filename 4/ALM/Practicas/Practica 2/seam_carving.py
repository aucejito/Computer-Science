# -*- coding: utf-8 -*-
#Ejecutar con -> python seam_carving.py BroadwayTowerSeamCarvingA.png 80%
...
def incremental_compute_gradient(grad,img,path):
    ...
    width, height = len(grad[0]), len(grad) #nos quedamos con la altura y la anchura de grad

    # first and last rows compute a different, simpler, gradient
    for y in (0, height-1): # just first and last rows
        for x in range(max(0,path[y]-2), min(path[y]+2,width-1)): # first and last columns are excluded
            grad[y][x] = abs(img[y][x-1] - img[y][x+1])
...
def dp_seam_carving(grad,mat):
    ...
    posMatriz = height-2
    filaanterior = mat[posMatriz]
    while filaanterior!= None:
        minimo,posicion = min((filaanterior[pos],pos) for pos in (posicion-1,posicion,posicion+1))
        posMatriz = posMatriz-1
        #print(posicion)
        path.insert(0, posicion)
        if posMatriz < 0:
            break
        else:
            filaanterior = mat[posMatriz]
...
if __name__ == "__main__":
    ...
    ncolumns  = sys.argv[2]
    #file_name = "BroadwayTowerSeamCarvingA.png"
    #ncolumns = "80%"
    # open image
