# -*- coding: utf-8 -*-
# python template_multi_seam.py BroadwayTowerSeamCarvingA.png 80% 80
...
def remove_seams(height,seam_paths, matrix):
    for y in range(0, height):
        fila_ordenada = sorted(actual[y] for actual in seam_paths)
        for x in reversed(fila_ordenada):
            matrix[y].pop(x)
...
def dp_seam_carving_multi(grad,mat,N,pscore=0.7):
    ...
    #Rellenamos la matriz mat, empezando por la segunda fila y la segunda columna
    for fila in range(1, height):
        for columna in range(1, width-1):
            minimo = min(mat[fila-1][pos] for pos in (columna-1,columna,columna+1))
            mat[fila][columna] = minimo + grad[fila][columna]
    
    #Recuperamos el camino
    ultimafila = mat[height-1]
    posicion = -1
    minActual = infty
    for columna,actual in enumerate(ultimafila):
        if actual < minActual:
            minActual = actual
            posicion = columna
    ...
    # COMPLETE HERE: DONE
    used = set()
    # Lista ordenada de tuplas con el score y su posición
    lista = sorted((mat[height-1][coord],coord) for coord in range(1,width-1))
    best_score = lista[0][0]
    score = lista[0][0]
    actual = 0
    while actual < len(lista) and len(paths) < N and score*pscore <= best_score:
        score = lista[actual][0]
        pos = lista[actual][1]
        actual = actual+1
        path = []
        for y in reversed(range(0,height)):
            if (y,pos) not in used:
                used.add((y,pos))
                path.append(pos)
                aux = [mat[y-1][pos-1],mat[y-1][pos],mat[y-1][pos+1]]
                min_val = min(aux)
                pos = pos + aux.index(min_val) -1
            else:
                break
        if len(path) == height:
            path.reverse()
