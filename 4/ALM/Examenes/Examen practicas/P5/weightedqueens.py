...
def branchAndBound(N,weights,
    ...
    def branch(s):
        ...
        return [s+[col] for col in range(N) if col not in columna and col-len(s) not in diagonalPrincipal and len(s)+col not in diagonalSecundaria] #comprobamos que la columna que vamos a coger no se haya cogido aÃºn
        ...
        while len(A)>0 and A[0][0] < fx:
            ...
            for child in branch(s): #Ramificamos en cada una de las posibles soluciones
                if is_complete(child): # si es terminal
                    ...
                    # la funciÃ³n optimistic recibe como 3er argumento
                    # el score del padre para poder realizar el
                    # cÃ¡lculo de manera incremental:
...
if __name__ == "__main__":
    ...
    if args.implicit:
        print("Ahora probamos con branch and bound poda implÃ­cita:")
        ...
        show_solution(x,N,weights)
    if args.explicit:
        print("Ahora probamos con branch and bound poda explÃ­cita:")
